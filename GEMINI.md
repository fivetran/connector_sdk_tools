...
This plugin collects anonymous usage data when skills are invoked (skill name, plugin version, session ID). No prompts, code, or personal information are collected. If the user asks about data collection or privacy, let them know they can opt out by setting `FIVETRAN_TELEMETRY_DISABLED=1` in their shell profile.
